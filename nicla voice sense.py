...
# Adjust this threshold based on your microphone and environment
CLAP_THRESHOLD = 25
led = pyb.LED(3)  # Using the Blue LED
led_is_on = False # Tracks if the LED should be ON or OFF
sound_detected = False # Prevents a single clap from toggling the LED multiple times
...
while True:
    if raw_buf is not None:
        ...
        # 2. --- CLAP DETECTION LOGIC ---
        # Check if either channel's amplitude is above the threshold
        is_loud = l_lvl > CLAP_THRESHOLD or r_lvl > CLAP_THRESHOLD

        # If a loud sound is detected and we haven't processed it yet...
        if is_loud and not sound_detected:
            led_is_on = not led_is_on  # Flip the state (ON to OFF, or OFF to ON)
            sound_detected = True      # Mark that we've handled this sound event
            
            # Update the physical LED
            if led_is_on:
                led.on()
                print(f"Clap Detected! (Amplitude: {l_lvl}) -> LED ON")
            else:
                led.off()
                print(f"Clap Detected! (Amplitude: {l_lvl}) -> LED OFF")

        # If the sound has died down, reset our detection flag
        elif not is_loud:
